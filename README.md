...
- [ ] Deep linking universel (`nexus-jdr.app/join/{code}` ouvre directement l'app) : le routage interne (`go_router`), la config Android/iOS (`AndroidManifest.xml`, `Runner.entitlements`) et les fichiers de vérification côté web (`assetlinks.json`, route `apple-app-site-association`) sont en place. Keystore Android de production généré et câblé (`android/app/build.gradle.kts` signe les builds release). **Reste bloqué sur 2 prérequis externes** : domaine `nexus-jdr.app` pas encore hébergé, pas de compte Apple Developer Program créé (bloque le Team ID pour `apple-app-site-association` et le raccordement de `Runner.entitlements` dans Xcode) — étapes détaillées dans `DEEP_LINKING_SETUP.md` du dépôt web.
