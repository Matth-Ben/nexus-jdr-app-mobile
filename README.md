...
- [x] Phase 2 — application mobile V1 :
  ...
  - [x] Montée de niveau (déclenchement automatique, popin d'annonce par niveau, choix selon la classe, PV, emplacements de sorts, récapitulatif, **multiclassage RAW complet** : prérequis de caractéristiques, maîtrises limitées, sous-classe/sorts de départ si applicable, emplacements de sorts combinés).
...
- [ ] Écrans de montée de niveau encore bloqués par manque de fonctionnalité (pas de contenu — voir Phase 5 ci-dessus, `feats`/`invocations`/sorts niveau 2+ sont peuplés) : choisir un nouveau sort pour les classes "à sorts connus" (Barde/Ensorceleur/Occultiste/Rôdeur), choisir un don en alternative à l'augmentation de caractéristiques, choisir une invocation occultiste.
