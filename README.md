...
- [x] Export XML compatible aidedd.org (inverse de l'import Phase 3, vérifié par aller-retour réel) — quelques champs non réversibles documentés (multiclassage limité à la classe primaire, PV réinitialisés au réimport, sous-classe/invocations non exportées : disponibles sur la fiche personnage depuis peu, mais l'export XML ne les consomme pas encore).
...
- [x] Sous-classe, choix de classe (style de combat/ennemi juré), invocations occultistes connues et historique personnalisé (`background_custom_text`, import XML) affichés sur la fiche personnage — gap de lecture trouvé en construisant l'export XML, comblé depuis (`CharacterRepository.fetchCharacterDetail` les relit désormais, nouvelles cartes "CHOIX DE CLASSE"/"INVOCATIONS CONNUES" de l'onglet Compétences). Bug corrigé au passage : un personnage importé XML sans sexe reconnu affichait la chaîne technique `"(absent)"` au lieu de "Non renseigné".
