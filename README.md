...
- [x] Phase 2 — application mobile V1 :
  ...
  - [x] Montée de niveau (déclenchement automatique, popin d'annonce par niveau, choix selon la classe, PV, emplacements de sorts, récapitulatif, **multiclassage RAW complet** : prérequis de caractéristiques, maîtrises limitées, sous-classe/sorts de départ si applicable, emplacements de sorts combinés) — plus **choisir un nouveau sort à tout niveau** pour les classes "à sorts connus" (Barde/Ensorceleur/Occultiste/Rôdeur), **choisir un don** en alternative à l'augmentation de caractéristiques (avec panneau "Infos"), **choisir une invocation** occultiste (quota effectif selon le contenu disponible).
