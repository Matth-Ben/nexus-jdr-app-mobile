...
- [x] Phase 2 — application mobile V1 :
  ...
  - [x] Fiche personnage complète, 5 onglets, tous avec des actions d'écriture câblées : Personnage (identité, caractéristiques, jets de sauvegarde, PV ajustables, apparence physique, portrait, repos court/long avec dépense de dés de vie RAW), Compétences (aptitudes, 18 compétences, outils/langues — lecture seule par nature), Sorts (lancer un sort, utiliser une aptitude, décompte d'emplacements/usages), Inventaire (équiper/utiliser/retirer un objet, ajuster la monnaie, ajouter un objet ou une récompense), Histoire (les 9 champs de texte libre éditables).
  - [x] Montée de niveau (déclenchement automatique, popin d'annonce par niveau, choix selon la classe, PV, emplacements de sorts, récapitulatif, **multiclassage RAW complet** : prérequis de caractéristiques, maîtrises limitées, sous-classe/sorts de départ si applicable, emplacements de sorts combinés, **choix de quelle classe progresse** à chaque montée de niveau ultérieure) — plus **choisir un nouveau sort à tout niveau** pour les classes "à sorts connus" (Barde/Ensorceleur/Occultiste/Rôdeur), **choisir un don** en alternative à l'augmentation de caractéristiques (avec panneau "Infos"), **choisir une invocation** occultiste (quota effectif selon le contenu disponible), **magie de pacte de l'Occultiste** (emplacements dédiés, rechargés au repos court et long). Maîtrises d'armures/armes affichées durablement dans l'onglet Compétences (classe primaire + classes secondaires multiclassées, fusionnées et dédupliquées).
...
- [x] Phase 4 — synchronisation avec l'app web "Histoires" : rejoindre une histoire par code d'invitation (4 étapes, avec le nom du MJ affiché dès la confirmation s'il en a renseigné un), section "Aventures" sur la fiche (nom du MJ inclus), MJ avec accès en lecture seule au personnage rattaché. Préalable côté dépôt web fait et déployé (colonnes `invite_code`/`invite_code_enabled`, table `character_campaigns` + RLS, edge functions `preview-story-invite`/`join-story`, colonne calculée `stories_gm_display_name`).
- [x] Phase 5 — extension du contenu D&D au-delà du strict Manuel des Joueurs : sorts (57 → 477, tous niveaux et toutes classes), sous-classes (12 → 103, chacune avec au moins une aptitude signature — texte entièrement reformulé pour les 91 sous-classes issues d'extensions hors licence SRD/CC-BY), 42 dons du Manuel des Joueurs (0 → 42, reformulés hors le seul "Empoignade" couvert par le SRD), 32 invocations occultistes (0 → 32, traduites fidèlement depuis le texte SRD/CC-BY officiel), historiques (13 → 15), races (9 → 13 : Aarakocra/Génasi/Goliath/Orc 2024) et sous-races (9 → 14), progression complète niveau par niveau des aptitudes de classe/sous-classe (148 → 649 lignes, contre une poignée d'aptitudes isolées suivies jusqu'ici), 2 nouvelles tables `race_lineages`/`racial_innate_spells` (ex. couleur de dragon du Drakéide, sorts innés raciaux).
- [x] Notifications push (Android) : préférences par déclencheur (rappel de repos long, accès à une histoire retiré) et résumé email hebdomadaire, écran dédié dans Profil. Backend FCM/Resend déployé mais l'envoi réel reste bloqué tant que l'utilisateur n'a pas configuré les secrets `FIREBASE_SERVICE_ACCOUNT_JSON`/`RESEND_API_KEY` sur le Dashboard Supabase.
- [x] Système de groupe : créer un groupe ou le rejoindre par code depuis l'app (sans MJ ni histoire requis), tableau de bord des coéquipiers (PV/statut vivant-inconscient-mort en temps réel via Supabase Realtime), butin commun (monnaie/objets, attribution vers l'inventaire personnel), gestion complète (renommer, régénérer le code, dissoudre, quitter, exclure).
- [x] Export XML compatible aidedd.org (inverse de l'import Phase 3, vérifié par aller-retour réel) — quelques champs non réversibles documentés (multiclassage limité à la classe primaire, PV réinitialisés au réimport, sous-classe/invocations non exportées faute d'être relues par la fiche personnage aujourd'hui).
- [x] Panneau web "Joueurs" (dépôt web `markdown-editor`) : génération/copie/régénération du lien d'invitation d'une histoire, liste des personnages rattachés, retrait d'un joueur.
...
- [ ] Notifications push iOS (bloqué sur le compte Apple Developer) ; déclencheur "invitation à rejoindre une histoire reçue" (le modèle d'invitation actuel, code partagé hors bande, n'a aucun événement serveur ciblant un joueur précis — nécessite des invitations nominatives, non construites).
- [ ] Lien "voir la fiche complète en lecture seule" d'un joueur depuis le panneau web "Joueurs" — aucune route de fiche personnage en lecture seule n'existe côté web (le "partage en lecture d'un personnage", `12-partage-et-groupes.md` section 1, n'est pas construit).
- [ ] Gap de lecture de la fiche personnage trouvé en construisant l'export XML : sous-classe, style de combat, ennemis favoris, historique personnalisé (`background_custom_text`), invocations occultistes connues sont bien écrits en base mais jamais relus par `CharacterRepository.fetchCharacterDetail` — les afficher durablement sur la fiche est un chantier à part.
- [ ] Bug préexistant sans impact utilisateur visible mais à corriger : `XmlImportSaveDataResolver` promet un repli "Non renseigné" pour un sexe non reconnu à l'import XML, mais du code mort empêche de l'atteindre — un personnage importé sans sexe reconnu affiche la chaîne technique `"(absent)"`.
- [ ] Phase 6 — fonctionnalités complémentaires restantes : reste du mode hors-ligne (au-delà de PV/XP) si le besoin se confirme.
