...
- [ ] Notifications push/email (`15-profil-parametres.md` section 3) — socle Firebase Cloud Messaging Android câblé (3 flavors, `firebase_messaging` en dépendance) mais pas encore utilisé côté application : reste la table de tokens d'appareil, l'edge function d'envoi (dépôt web, bloquée sur le secret `FIREBASE_SERVICE_ACCOUNT_JSON` à ajouter par l'utilisateur), le câblage mobile (demande de permission, enregistrement du token), un écran de préférences, et l'arbitrage produit (contenu, fréquence). iOS bloqué sur le compte Apple Developer.
